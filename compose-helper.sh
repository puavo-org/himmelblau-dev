...
BASE_URL="https://packages.himmelblau-idm.org/$CHANNEL/$HB_VERSION/deb/debian12"

mkdir -p "$ROOTFS_DIR/tmp/hb_debs"
CORE_PACKAGES="himmelblau nss-himmelblau pam-himmelblau himmelblau-sso"

for PKG in $CORE_PACKAGES; do
  file="${PKG}_${HB_VERSION%%-*}-debian12_amd64.deb"
  curl -fsSL -o "$ROOTFS_DIR/tmp/hb_debs/$file" "$BASE_URL/$file"
done

dpkg --root "$ROOTFS_DIR" --force-script-chrootless --install "$ROOTFS_DIR"/tmp/hb_debs/*.deb || true
